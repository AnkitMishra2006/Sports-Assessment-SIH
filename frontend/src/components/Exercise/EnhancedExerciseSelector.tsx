...
export function EnhancedExerciseSelector({ onStartAnalysis }: ExerciseSelectorProps) {
  ...
  const selectedExerciseData = exercises.find(e => e.value === selectedExercise);
    ...
    <div className="w-full max-w-6xl mx-auto space-y-6">
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Get real-time feedback on your exercise form and performance using advanced computer vision
        </p>
      </div>
            ...
            {exercises.map((exercise) => (
              ...
              >
                <CardContent className="p-6">
                  <div className={`w-full h-24 bg-gradient-to-r ${exercise.color} rounded-lg mb-4 flex items-center justify-center`}>
                    <exercise.icon className="h-10 w-10 text-white" />
                  </div>
                  
                  <h3 className="font-semibold text-lg mb-2">{exercise.label}</h3>
      ...
      {selectedExercise && (
        <Card>
          <CardHeader>
            <CardTitle className="text-xl">Analysis Method</CardTitle>
            <p className="text-muted-foreground">
              Choose how you'd like to perform your exercise analysis
            </p>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              >
                <CardContent className="p-6 text-center">
                  ...
                  <h3 className="font-semibold text-lg mb-2">Live Analysis</h3>
          ...
          <CardContent className="p-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                <div className={`w-12 h-12 bg-gradient-to-r ${selectedExerciseData.color} rounded-lg flex items-center justify-center`}>
                  <selectedExerciseData.icon className="h-6 w-6 text-white" />
                </div>
                <div>
                  <h3 className="font-semibold text-lg">{selectedExerciseData.label}</h3>
                  <p className="text-sm text-muted-foreground">
                    {analysisMode === 'live' ? 'Live Camera Analysis' : 'Video Upload Analysis'}
                  </p>
                </div>
              </div>
